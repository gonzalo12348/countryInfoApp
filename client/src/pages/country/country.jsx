...
export default function CountryInfo() {
  ...
  useEffect(() => {
    if (iso2Code && countryName) {
      axios
        .get(`${process.env.REACT_APP_BACKEND_URL}/country`, {
          params: {
            iso2: iso2Code,
            name: countryName,
          },
  ...
  if (!countryInfo)
    return <p className="loading-message">Loading country information...</p>;
  ...
  return (
    <div className="country-info-container">
      ...
      <h1 className="country-name">
        {countryInfo.name || countryInfo.name.officialName}
      </h1>
      <img
      ...
      <ul className="borders-list">
        {countryInfo.borders.map((borderCountry) => (
          <Link
            to={`/country?iso2=${borderCountry.countryCode}&name=${borderCountry.commonName}`}
            className="border-country-link"
            key={borderCountry.countryparam}
          >
